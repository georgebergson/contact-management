    ...
    <form class="w-full mx-auto space-y-3" method="POST" action="{{ route('contacts.store') }}">
        ...
        <div class="contact">
            ...
            <div class="flex">
                ...
                <input type="tel" id="contact" name="contact" minlength="9" maxlength="9" class="rounded-none rounded-e-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="(00) 0000-0000">
            </div>
            @error('contact')
            <div class="text-red-500 text-xs mt-1">{{ $message }}</div>
            @enderror
